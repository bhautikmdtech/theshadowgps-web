...
export default function SocketChecker() {
  ...
  const [selectedEvent, setSelectedEvent] = useState<string>("requestLiveTripData");
  ...
  const handleConnect = (e: FormEvent) => {
    ...
    try {
      ...
      newSocket.on("connect", () => {
        setIsConnected(true);
        setError(null);
        setResponses(prev => [...prev, { type: "connection", data: "Socket connected successfully" }]);
      });

      newSocket.on("connect_error", (err) => {
        setError(err.message);
        setIsConnected(false);
        setResponses(prev => [...prev, { type: "error", data: `Connection error: ${err.message}` }]);
      });
      ...
      setError(err.message);
      setResponses(prev => [...prev, { type: "error", data: `Error: ${err.message}` }]);
    }
  };
  ...
  const handleSendRequest = (e: FormEvent) => {
    ...
    try {
      ...
      if (selectedEvent === "requestLiveTripData" || selectedEvent === "unsubscribeLiveTripData") {
  ...
  return (
    <div className="container mx-auto p-4 max-w-6xl bg-white dark:bg-gray-800 min-h-screen text-black dark:text-white">
      ...
      {/* Connection Form */}
      <form onSubmit={handleConnect} className="mb-6 p-6 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold mb-4 text-center">Socket Connection</h2>
        <div className="mb-4">
          <label htmlFor="socketUrl" className="block mb-2 font-medium">Socket URL</label>
          <input
            type="text"
            id="socketUrl"
            value={socketUrl}
        ...
        <div className="mb-6">
          <label htmlFor="token" className="block mb-2 font-medium">Auth Token</label>
          <input
            type="text"
            id="token"
            value={token}
      ...
      {/* Request Form */}
      <form onSubmit={handleSendRequest} className={`mb-6 p-6 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-md ${!isConnected ? 'opacity-50' : ''}`}>
        <h2 className="text-xl font-semibold mb-4 text-center">Send Request</h2>
        
        <div className="mb-4">
          <label htmlFor="selectedEvent" className="block mb-2 font-medium">Event Type</label>
          <select
            id="selectedEvent"
            value={selectedEvent}
        ...
        {(selectedEvent === "requestLiveTripData" || selectedEvent === "unsubscribeLiveTripData") && (
          <div className="mb-6">
            <label htmlFor="requestIdent" className="block mb-2 font-medium">Ident Parameter</label>
            <input
              type="text"
              id="requestIdent"
              value={requestIdent}
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
              Will be sent as: {"{ ident: \"" + (requestIdent || "value") + "\" }"}
            </p>
          </div>
        )}
